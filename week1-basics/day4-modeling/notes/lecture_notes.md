...
- compress=3 vs 9: 용량 차이 미미 (0.4MB)
- compress=0 vs 3: 저장 100배, 로드 20배 차이
- 속도 vs 용량 트레이드오프 존재

**결론**:

- 서빙 환경 → compress=0 (로드 속도 우선)
- 스토리지 절약 → compress=3
- compress=9는 거의 안 씀

**백엔드 비유**:

- compress=0 = Redis 캐시 (빠름)
- compress=3 = S3 장기 보관 (압축)

- 보완
  | compress | save   | load   | size   | 추천 상황     |
  | -------- | ------ | ------ | ------ | ------------- |
  | 0        | 0.028s | 0.019s | 76.3MB | 서빙 환경     |
  | 3        | 3.193s | 0.512s | 68.9MB | 스토리지 절약 |
  | 9        | 3.548s | 0.474s | 68.5MB | 거의 안 씀    |
  개발/실험 단계: compress=3 (용량 절약, 저장 빈도 낮음)
  joblib.dump(model, "model.joblib", compress=3)
  프로덕션 서빙: compress=0 (로드 속도 최우선)
  joblib.dump(model, "model.joblib", compress=0)
  S3 업로드용: compress=3

### [WHAT IF] random_state 없애면?

- 실험 결과: 매번 할때마다 다른 값이 나옴
- 결론: 랜덤한 값을 고정 하는 seed이므로 재현성을 유지 하려면 해당 시드를
  config같은 곳에 저장해두고, 균일하게 사용해야 할듯

### [WHY] F1-Score vs Accuracy

- Accuracy 함정 예시: 소수 데이터 일때
- 실무 선택 기준:
  데이터가 균일하고 직관적일때는 Accuracy 사용해도될거 같은데
  물론 뭐에 비용 즉, 리스크를 덜 감수 할거냐에 따라 F1-Score(recall, precision 적절한 조합) 사용여부도 생각해야할듯
